...
import { loadPrivateKeyFromEnvOrFile, computeJengaSignatureBase64, validateSigningEnvs } from "./utils/signing";
...
export const createPaymentRecord = action({
    ...
    handler: async (ctx: any, args: any): Promise<{ paymentId: string; paymentData: any }> => {
        const now = Date.now();
        ...
        const signature = privateKey ? computeJengaSignatureBase64(signatureData, privateKey) : "";
        ...
        const paymentId: string = await ctx.runMutation(internal.orders.insertPaymentRecord, { paymentData });
