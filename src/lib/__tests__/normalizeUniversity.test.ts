...
describe('normalizeUniversity', () => {
  it('normalizes strings', () => {
    expect(normalizeString(' University of Nairobi ')).toBe('university of nairobi');
    expect(normalizeString("UoN.")).toBe('uon');
  });

  it('matches exact canonical names', () => {
    expect(matchUniversity('University of Nairobi', canonical)).toBe('University of Nairobi');
  });
